...
void CustomPowerupModel::init_(Player* target) {   
    ...
    this->headBoneID = mdl->bodyModels[mdl->currentPowerupModel]->model->searchBoneIndex("face_1");
    this->rootBoneID = mdl->bodyModels[mdl->currentPowerupModel]->model->searchBoneIndex("skl_root");

    this->hammerHelmet = ModelWrapper::create("obj_coin", "obj_coin");
    this->hammerShell = ModelWrapper::create("obj_coin", "obj_coin");
}
...
void CustomPowerupModel::draw_(Player* target) {
    switch (target->powerupState) {
        case PlayerBase::PowerupState::Hammer: {
            ...
            mdl->model->getBoneWorldMatrix(this->headBoneID, &mtx);
            ...
            mdl->model->getBoneWorldMatrix(this->rootBoneID, &mtx);
