...
class ParaBones : public Enemy {
    SEAD_RTTI_OVERRIDE(ParaBones, Enemy);
...
const ActorInfo ParaBonesActorInfo = { sead::Vector2i(8, -16), sead::Vector2i(0, 16), sead::Vector2i(8, 16), 0, 0, 0, 0, 0 };
...
const HitboxCollider::Info ParaBones::sCollisionInfo = {
    sead::Vector2f(0.0f, 8.0f), sead::Vector2f(8.0f, 12.0f), HitboxCollider::Shape::Rectangle, 3, 0, 0xFFFFFFFF, 0xFFFBFFFF, 0, &Enemy::collisionCallback
};
...
u32 ParaBones::onExecute() {
    ...
    sead::Matrix34f mtxWings;
    mtxWings.makeRTIdx(this->rotation, this->position + sead::Vector3f(0.0f, 8.0f, 0.0f));
...
void ParaBones::collisionPlayer(HitboxCollider* hcSelf, HitboxCollider* hcOther) {
    ...
    if (hitType == Enemy::HitType::Collide) {
        ...
        this->killPlayerJump(hcOther->owner, sead::Vector2f(0.0f, 0.0f), &ParaBones::StateID_Die);
    }
}
...
void ParaBones::beginState_Die() {
    ...
    DryBones.position = this->position + sead::Vector3f(0.0f, -8.0f, 0.0f);
