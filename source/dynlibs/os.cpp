...
//!----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
EXPORT_DECL(s32, OSDynLoadAcquire, const char* rpl, u32 *handle);
EXPORT_DECL(s32, OSDynLoadFindExport, u32 handle, s32 isdata, const char *symbol, void *address);

EXPORT_DECL(void, OSDynLoadRelease, u32 handle);
...
EXPORT_DECL(void *, OSSetExceptionCallback, u8 exceptionType, exceptionCallback newCallback);
EXPORT_DECL(void *, OSSetExceptionCallbackEx, s32 unkwn, u8 exceptionType, exceptionCallback newCallback);
...
EXPORT_DECL(s32, __osSnprintf, char* s, s32 n, const char * format, ...);
EXPORT_DECL(s32 *, __ghErrnoPtr, void);
...
//! MCP functions
//!----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
EXPORT_DECL(s32, MCPOpen, void);
EXPORT_DECL(s32, MCPClose, s32 handle);
EXPORT_DECL(s32, MCPTitleCount, s32 handle);
EXPORT_DECL(s32, MCPTitleList, s32 handle, s32 *res, void *data, s32 count);
EXPORT_DECL(s32, MCPGetOwnTitleInfo, s32 handle, void *data);
EXPORT_DECL(void*, MCPGetDeviceId, s32 handle, u32 * id);
...
EXPORT_DECL(void, addrLiWaitOneChunk, void);
EXPORT_DECL(void, addrSgIsLoadingBuffer, void);
EXPORT_DECL(void, addrGDynloadInitialized, void);

//!----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
//! Kernel function addresses
//!----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
EXPORT_DECL(void, addrPrepareTitleHook, void);

//!----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
//! Other function addresses
//!----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
EXPORT_DECL(void, DCInvalidateRange, void *buffer, u32 length);
EXPORT_DECL(s32, OSDynLoadGetModuleName, s32 handle, char *nameBuffer, s32 *nameBufferSize);
...
EXPORT_DECL(s32, IOSIoctl,s32 fd, u32 request, void *inputBuffer,u32 inputBufferLen, void *outputBuffer, u32 outputBufferLen);
EXPORT_DECL(s32, IOSIoctlAsync,s32 fd, u32 request, void *inputBuffer,u32 inputBufferLen, void *outputBuffer, u32 outputBufferLen, void *cb, void *cbarg);
EXPORT_DECL(s32, IOSOpen,char *path, u32 mode);
EXPORT_DECL(s32, IOSClose,s32 fd);

void _os_find_export(u32 handle, const char *funcName, void *funcPointer) {
    OSDynLoadFindExport(handle, 0, funcName, funcPointer);
...
void InitAcquireOS(void) {
    ...
    //! Lib handle functions
    //!----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    EXPORT_FUNC_WRITE(OSDynLoadAcquire, (s32 (*)(const char*, unsigned *))OS_SPECIFICS->addrOSDynLoadAcquire);
    EXPORT_FUNC_WRITE(OSDynLoadFindExport, (s32 (*)(u32, s32, const char *, void *))OS_SPECIFICS->addrOSDynLoadFindExport);

    OSDynLoadAcquire("coreinit.rpl", &coreinitHandle);
}
...
void InitOSFunctionPointers(void) {
    ...
    OS_FIND_EXPORT(coreinitHandle, OSDynLoadRelease);
    ...
    OS_FIND_EXPORT(coreinitHandle, __osSnprintf);
    OS_FIND_EXPORT(coreinitHandle, __ghErrnoPtr);

    OSDynLoadFindExport(coreinitHandle, 0, "_Exit", &__Exit);
    ...
    //! MCP functions
    //!----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    OS_FIND_EXPORT(coreinitHandle, MCPOpen);
    OS_FIND_EXPORT(coreinitHandle, MCPClose);
    OS_FIND_EXPORT(coreinitHandle, MCPTitleCount);
    OS_FIND_EXPORT(coreinitHandle, MCPTitleList);
    OS_FIND_EXPORT(coreinitHandle, MCPGetOwnTitleInfo);
    OS_FIND_EXPORT(coreinitHandle, MCPGetDeviceId);

    //!----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    //! Memory functions
    //!----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    OSDynLoadFindExport(coreinitHandle, 1, "MEMAllocFromDefaultHeapEx", &pMEMAllocFromDefaultHeapEx);
    OSDynLoadFindExport(coreinitHandle, 1, "MEMAllocFromDefaultHeap", &pMEMAllocFromDefaultHeap);
    OSDynLoadFindExport(coreinitHandle, 1, "MEMFreeToDefaultHeap", &pMEMFreeToDefaultHeap);
    ...
    OS_FIND_EXPORT(coreinitHandle, OSDynLoadGetModuleName);
    ...
    //! IOS functions
    //!----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    OS_FIND_EXPORT(coreinitHandle, IOSIoctl);
    OS_FIND_EXPORT(coreinitHandle, IOSIoctlAsync);
    OS_FIND_EXPORT(coreinitHandle, IOSOpen);
    OS_FIND_EXPORT(coreinitHandle, IOSClose);
}
