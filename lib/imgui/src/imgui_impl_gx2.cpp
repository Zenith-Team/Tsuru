...
// Include shader data
#include "imgui/shaders/shader.h"
...
static void ImGui_ImplGX2_SetupRenderState(ImDrawData* draw_data, int fb_width, int fb_height)
{
    ...
    GX2SetDepthOnlyControl(FALSE, FALSE, GX2_COMPARE_NEVER);
...
void    ImGui_ImplGX2_RenderDrawData(ImDrawData* draw_data)
{
    ...
    // Flush memory
    GX2Invalidate(GX2_INVALIDATE_CPU_ATTRIB_BUFFER, bd->VertexBuffer, vtx_buffer_size);
    GX2Invalidate(GX2_INVALIDATE_CPU, bd->IndexBuffer, idx_buffer_size);
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                GX2DrawIndexedEx(GX2_PRIMITIVE_TRIANGLES, pcmd->ElemCount,
                    sizeof(ImDrawIdx) == 2 ? GX2_INDEX_FORMAT_U16 : GX2_INDEX_FORMAT_U32,
...
bool ImGui_ImplGX2_CreateFontsTexture()
{
    ...
    tex->compSel = GX2_COMP_MAP(GX2_SQ_SEL_A, GX2_SQ_SEL_B, GX2_SQ_SEL_G, GX2_SQ_SEL_R);

    GX2RCreateSurface(&tex->surface, GX2R_RESOURCE_BIND_TEXTURE | GX2R_RESOURCE_USAGE_CPU_WRITE | GX2R_RESOURCE_USAGE_GPU_READ);
    GX2InitTextureRegs(tex);

    unsigned char* dst_pixels = (unsigned char*) GX2RLockSurfaceEx(&tex->surface, 0, 0);
    ...
    GX2InitSampler(bd->FontTexture->Sampler, GX2_TEX_CLAMP_CLAMP, GX2_TEX_XY_FILTER_BILINEAR);
...
void ImGui_ImplGX2_DestroyFontsTexture()
{
    ...
    if (bd->FontTexture)
    {
        GX2RDestroySurfaceEx(&bd->FontTexture->Texture->surface, 0);
        io.Fonts->SetTexID(0);
...
bool    ImGui_ImplGX2_CreateDeviceObjects()
{
    ...
    WHBGfxInitShaderAttribute(bd->ShaderGroup, "Position", 0, 0, GX2_ATTRIB_FORMAT_32_32_FLOAT);
    WHBGfxInitShaderAttribute(bd->ShaderGroup, "UV", 0, 8, GX2_ATTRIB_FORMAT_32_32_FLOAT);
    WHBGfxInitShaderAttribute(bd->ShaderGroup, "Color", 0, 16, GX2_ATTRIB_FORMAT_8_8_8_8_UNORM);
