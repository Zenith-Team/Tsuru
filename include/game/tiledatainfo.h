...
public:
    ...
    u64 getUnitType() const {
        return this->data & 0xFFFFFFFFF;
    }

    Kind getUnitKind() const {
        return Kind(this->data & 0xFFFF);
    }
    ...
    SolidType getUnitSolidType() const {
        return SolidType((this->data >> 32) & 0xF);
    }

    SlideType getUnitSlideType() const {
        return SlideType((this->data >> 36) & 0xF);
    }

    SurfaceType getUnitSurfaceType() const {
        return SurfaceType((this->data >> 40) & 0xFF);
    }
...
static_assert(sizeof(TileDataInfo) == 0x8, "TileDataInfo size mismatch");
